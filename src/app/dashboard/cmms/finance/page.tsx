...
export default function CmmsFinanceDashboardPage() {
  ...
  const financeData = React.useMemo(() => {
    ...
    const clientPayables = allAccountsPayable; // Assuming payables are for the whole SaaS company for now, not client-specific. This can be adjusted.
    ...
    allEntries.forEach(entry => {
        ...
        if ('customerLocationId' in entry) { // It's an AccountsReceivable
            if (entry.status === 'Paga') monthlyData[monthYear].entries += entry.value;
        } else { // It's an AccountsPayable
            if (entry.status === 'Paga') monthlyData[monthYear].exits += entry.value;
        }
    });

    // Add contract revenue to monthly entries for a more complete picture
    ...
    return { totalReceivable, totalPayable, balance, nextEntries, cashFlow };

  }, [selectedClient]);

  const { totalReceivable, totalPayable, balance, nextEntries, cashFlow } = financeData;
